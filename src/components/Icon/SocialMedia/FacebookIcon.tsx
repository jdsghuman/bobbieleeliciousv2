...
const FacebookIcon = ({ fill, link, position, className, viewBox }: IconPropType) => {
  return (
    <div className={className}>
      <a className={styles.icon__link} href={link} target="_blank" rel="noopener noreferrer">
        {position === 'navbar' ? (
          <svg
            aria-labelledby="facebook-id"
            viewBox={viewBox}
            fill={fill}
            xmlns="http://www.w3.org/2000/svg"
          >
            <title id="facebook-id">facebook for Bobbieleelicious</title>
